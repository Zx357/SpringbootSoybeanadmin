...
const { baseURL: appBaseApi } = getServiceBaseURL(import.meta.env, isHttpProxy)
...
const headers = { Authorization: `Bearer ${localStg.get('token') || ''}` }

const uploadUrl = computed(() => {
  return appBaseApi + props.action + '?updateSupport=' + (updateSupport.value ? 1 : 0)
})
