...
-- 初始化-公告信息表数据
-- ----------------------------
insert into sys_notice values('1', '欢迎使用 RuoYi-SoybeanAdmin 管理系统', '1', '<p>欢迎使用 RuoYi-SoybeanAdmin！</p><p>本项目将若依（RuoYi-Vue）的后台管理前端整体迁移并美化至 SoybeanAdmin（Element Plus 版）模板：后端接口与权限体系零改动，前端界面全面焕新。</p><p>当前版本 v1.0.0 已包含系统管理、系统监控、系统工具全部功能模块。</p>', '0', 'admin', sysdate(), '', null, '项目欢迎公告');
insert into sys_notice values('2', '系统使用指引', '2', '<p><b>常用功能入口：</b></p><p>1. 系统管理：用户 / 角色 / 菜单 / 部门 / 岗位 / 字典 / 参数 / 通知公告 / 日志管理</p><p>2. 系统监控：在线用户 / 定时任务 / 数据监控 / 服务监控 / 缓存监控</p><p>3. 系统工具：表单构建 / 代码生成 / 系统接口文档</p><p><b>界面小贴士：</b>右上角可切换暗黑模式、全屏；主题配置抽屉支持多布局模式与主题色定制。</p>', '0', 'admin', sysdate(), '', null, '功能导航说明');
insert into sys_notice values('3', 'RuoYi-SoybeanAdmin v1.0.0 正式发布', '2', '<p><b>v1.0.0 更新内容：</b></p><p>1. 若依全部前端功能迁移至 SoybeanAdmin 模板，逐页适配并实测通过</p><p>2. 动态路由对接：菜单由后端 /getRouters 接口下发</p><p>3. 按钮级权限指令 v-hasPermi / v-hasRole 完整保留</p><p>4. 界面焕新：多布局模式、暗黑模式、主题色定制、精致多标签页</p><p>项目地址：https://gitee.com/zxenglish/springboot-soybeanadmin</p>', '0', 'admin', sysdate(), '', null, '版本发布公告');
